...
var import_mume3 = __toModule(require("@shd101wyy/mume"));
var import_coc19 = __toModule(require("coc.nvim"));
var import_path2 = __toModule(require("path"));

// src/image-helper.ts
var import_mume = __toModule(require("@shd101wyy/mume"));
  ...
  fs.mkdir(assetDirectoryPath, () => {
    fs.stat(destPath, (err) => {
      if (err == null) {
        ...
        return import_coc16.window.showErrorMessage(err.toString());
      }
...
var mume = __toModule(require("@shd101wyy/mume"));
var import_mume2 = __toModule(require("@shd101wyy/mume"));
var import_utility = __toModule(require("@shd101wyy/mume/out/src/utility"));
...
`)
  ...
  updateMarkdown(sourceUri, triggeredBySave) {
    ...
    const doc = import_coc18.workspace.getDocument(sourceUri.fsPath);
    const text = doc.getDocumentContent();
...
async function openInVim(uri, type) {
  ...
  const escapedPath = await import_coc19.workspace.nvim.call("fnameescape", [uri.fsPath]);
...
function activate(context) {
  ...
  async function insertNewSlide() {
    const doc = await import_coc19.workspace.document;
    await doc.applyEdits([import_coc19.TextEdit.insert(await cursorPosition(), "<!-- slide -->\n")]);
  }
  async function insertPagebreak() {
    const doc = await import_coc19.workspace.document;
    await doc.applyEdits([import_coc19.TextEdit.insert(await cursorPosition(), "<!-- pagebreak -->\n")]);
  }
...
`)
  ...
  async function insertImageUrl(uri, imageUrl) {
    ...
    if (doc && isMarkdownFile(doc.textDocument)) {
      await doc.applyEdits([import_coc19.TextEdit.insert(await cursorPosition(), `![enter image description here](${imageUrl})`)]);
    }
  }
  ...
  async function clickTagA(uri, href) {
    ...
    const resourceUri = util2.ResourceUri.parse(href);
    const fsPath = resourceUri == null ? void 0 : resourceUri.fsPath;
    if (!fsPath) {
      return util2.openUri(href);
    }
    if ([".pdf", ".xls", ".xlsx", ".doc", ".ppt", ".docx", ".pptx"].indexOf(import_path2.default.extname(fsPath)) >= 0) {
      util2.openUri(href);
    } else {
      await openInVim(import_coc19.Uri.file(fsPath), openType);
    }
  }
  ...
  context.subscriptions.push(registerCommand("markdown-preview-enhanced.openPreview", () => openPreview(true)), registerCommand("markdown-preview-enhanced.openPreviewBackground", () => openPreview(false)), registerCommand("markdown-preview-enhanced.toggleScrollSync", toggleScrollSync), registerCommand("markdown-preview-enhanced.toggleLiveUpdate", toggleLiveUpdate), registerCommand("markdown-preview-enhanced.toggleBreakOnSingleNewLine", toggleBreakOnSingleNewLine), registerCommand("markdown-preview-enhanced.openImageHelper", openImageHelper), registerCommand("markdown-preview-enhanced.runAllCodeChunks", runAllCodeChunksCommand), registerCommand("markdown-preview-enhanced.runCodeChunk", runCodeChunkCommand), registerCommand("markdown-preview-enhanced.syncPreview", syncPreview), registerCommand("markdown-preview-enhanced.customizeCss", customizeCSS), registerCommand("markdown-preview-enhanced.openMermaidConfig", openMermaidConfig), registerCommand("markdown-preview-enhanced.openMathJaxConfig", openMathJaxConfig), registerCommand("markdown-preview-enhanced.openKaTeXConfig", openKaTeXConfig), registerCommand("markdown-preview-enhanced.extendParser", extendParser), registerCommand("markdown-preview-enhanced.showUploadedImages", showUploadedImages), registerCommand("markdown-preview-enhanced.insertNewSlide", insertNewSlide), registerCommand("markdown-preview-enhanced.insertTable", insertTable), registerCommand("markdown-preview-enhanced.insertPagebreak", insertPagebreak), registerCommand("markdown-preview-enhanced.createTOC", createTOC), registerCommand("_mume.revealLine", revealLine), registerCommand("_mume.insertImageUrl", insertImageUrl), registerCommand("_mume.pasteImageFile", pasteImageFile), registerCommand("_mume.uploadImageFile", uploadImageFile), registerCommand("_mume.refreshPreview", refreshPreview), registerCommand("_mume.openInBrowser", openInBrowser), registerCommand("_mume.htmlExport", htmlExport), registerCommand("_mume.chromeExport", chromeExport), registerCommand("_mume.princeExport", princeExport), registerCommand("_mume.eBookExport", eBookExport), registerCommand("_mume.pandocExport", pandocExport), registerCommand("_mume.markdownExport", markdownExport), registerCommand("_mume.webviewFinishLoading", webviewFinishLoading), registerCommand("_mume.cacheCodeChunkResult", cacheCodeChunkResult), registerCommand("_mume.runCodeChunk", runCodeChunk), registerCommand("_mume.runAllCodeChunks", runAllCodeChunks), registerCommand("_mume.clickTagA", clickTagA), registerCommand("_mume.clickTaskListCheckbox", clickTaskListCheckbox), registerCommand("_mume.showUploadedImageHistory", showUploadedImages), registerCommand("_mume.setPreviewTheme", setPreviewTheme));
}
